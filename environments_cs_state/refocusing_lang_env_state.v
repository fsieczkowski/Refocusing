...
Module Type RED_LANG.
  ...
  Axiom value_to_closure_injective : 
    forall v v0, value_to_closure v = value_to_closure v0 -> v = v0.
  Axiom redex_to_closure_injective : 
    forall r r0, redex_to_closure r = redex_to_closure r0 -> r = r0.

  Axiom valueC_to_closureC_injective :
    forall v v0, valueC_to_closureC v = valueC_to_closureC v0 -> v = v0.

  Axiom closureC_to_closure_injective :
    forall c c0, closureC_to_closure c = closureC_to_closure c0 -> c = c0.
  Axiom valueC_to_value_injective :
    forall v v0, valueC_to_value v = valueC_to_value v0 -> v = v0.
  Axiom frameC_to_frame_injective :
    forall f f0, frameC_to_frame f = frameC_to_frame f0 -> f = f0.

  Axiom pairC_pair : forall t sC,
    closureC_to_closure (pairC t sC) = pair t (map closureC_to_closure sC).
  Axiom pairC_injective : forall t t0 sC sC0, 
    pairC t sC = pairC t0 sC0 -> t = t0 /\ sC = sC0.
  Axiom valueC_to_closure_commutes : forall vC, 
    value_to_closure (valueC_to_value vC) = 
      closureC_to_closure (valueC_to_closureC vC).
  Definition envC_to_env : envC -> env := map closureC_to_closure.
  Definition envC_to_env_injective : forall sC sC0, 
    envC_to_env sC = envC_to_env sC0 -> sC = sC0 :=
  ...
  Parameter state : Set.
  Parameter stateC : Set.
  Parameter stateC_to_state : stateC -> state.
  Axiom stateC_to_state_injective : forall G G0
    (GEQ : stateC_to_state G = stateC_to_state G0),
    G = G0.
  Parameter init : state.
  Parameter initC : stateC.
  Axiom initC_init : stateC_to_state initC = init.

  Definition answer : Set := (value * state)%type.
  Definition answerC : Set := (valueC * stateC)%type.
  Definition answerC_to_answer aC :=
    match aC with (vC, GC) => (valueC_to_value vC, stateC_to_state GC) end.

  Parameter contract : redex -> context -> state -> 
    option (closure * context * state).

  Delimit Scope red_scope with reds.
  ...
  Axiom value_redex : forall (v : value) (r : redex),
  ...
  Axiom dec_pairC : forall t sC, 
    proj1_sig (closureC_dec (pairC t sC)) = inl _ (t, sC).

  (** Datatype of decompositions -- either a value or a redex in a
      context (analogous to the decompose lemma) *)
...
Module Type LANG_PROP (R : RED_LANG).
  ...
  Axiom plug_compose : forall (E E0 : context) (c : closure),
    E · E0 [ c ] = E0 [ E [ c ] ].
  Axiom composeC_compose : forall Ec Ec0,
...
Module Type RED_REF_LANG.
  ...
  | scl : forall cl cl0 f (AP : cl = atom_plug cl0 f), 
  ...
  Axiom dec_closure_red_empty  : forall c r,
    dec_closure c = in_red r -> only_empty c.
  Axiom dec_closure_val_empty  : forall c v,
    dec_closure c = in_val v -> only_empty c.
  Axiom dec_closure_clo_top    : forall c c0 f,
    dec_closure c = in_clo c0 f -> forall f0, ~f <: f0.
  Axiom dec_context_red_bot    : forall f v r,
    dec_context f v = in_red r -> forall f0, ~ f0 <: f.
  Axiom dec_context_val_bot    : forall f v v0,
    dec_context f v = in_val v0 -> forall f0, ~ f0 <: f.
  Axiom dec_context_clo_next   : forall f f0 v c,
    dec_context f v = in_clo c f0 -> f0 <: f /\ 
      forall f1, f1 <: f -> ~f0 <: f1.

  Axiom dec_closure_correct : forall c,
    match dec_closure c with
      | in_red r => redex_to_closure r = c
      | in_val v => value_to_closure v = c
      | in_clo c0 f =>  f [ c0 ]a = c
    end.
  Axiom dec_context_correct : forall f v, 
    match dec_context f v with
      ...
      | in_clo c f0 => f0 [ c ]a         = f [ value_to_closure v ]a
    end.

  Axiom frame_order_antisym : forall f f0, f <: f0 -> ~ f0 <: f.
  Axiom dec_frame_ord : forall c c0 f f0, 
    f[ c ]a = f0[ c0 ]a -> f <: f0 \/ f0 <: f \/ (c = c0 /\ f = f0).
  Axiom frame_det : forall c c0 f f0, 
    f <: f0 -> f[ c ]a = f0[ c0 ]a -> exists v, c0 = value_to_closure v.
...
End RED_REF_LANG.
  ...
  Ltac prove_st_wf := intro a; constructor; induction a; 
    (intros y H; inversion H as [c c0 f DECT]; subst; destruct f; 
      inversion DECT; subst; constructor; auto).
  Ltac prove_ec_wf := intro a; destruct a; 
    repeat (constructor; intros f T; destruct f; inversion T; subst; clear T).
